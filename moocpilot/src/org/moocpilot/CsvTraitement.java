...
public class CsvTraitement {
    ...
    */
    ...
    public CsvTraitement(ArrayList<String> paths, ArrayList<String> csvListName) {
        MoocPilotLogger.LOGGER.log(Level.INFO, "CsvTraitement.CsvTraitement, csvListName=" + csvListName);
...
/*				
  ...
  //~ MoocPilotLogger.LOGGER.log(Level.INFO,"   note1 "+columnWeek1.get(i)+": ("+rec.get(columnWeek1.get(i))+")"+note+", max "+noteW1);
				}
				for(int i=0; i<columnWeek2.size(); i++) {
					double note = Double.parseDouble(rec.get(columnWeek2.get(i)));
					if (noteW2 < note) noteW2 = note;
  //~ MoocPilotLogger.LOGGER.log(Level.INFO,"   note2 "+columnWeek2.get(i)+"("+rec.get(columnWeek2.get(i))+"): "+note+", max "+noteW2);
				}
				if (certifOK) Nok++;
				if (noteW1>0.0) N1++;
...
//MoocPilotLogger.LOGGER.log(Level.INFO,"   "+rec.get(columnUsername)+", note ("+noteW1+", "+noteW2+"), grade:"+rec.get(columnGrade)+", Ns "+Nok+","+N1+","+N2+","+N1ok+","+N2ok);
			}
//~ MoocPilotLogger.LOGGER.log(Level.INFO,"CertificateEligible: "+Nok+" / inscrit: "+N);
MoocPilotLogger.LOGGER.log(Level.INFO,"   listColumnUtilsName="+this.listColumnUtilsName+" -> resTab="+Arrays.toString(resTab)+", listColumnNoteName="+this.listColumnNoteName);
*/
            ...
            MoocPilotLogger.LOGGER.log(Level.INFO, e.getMessage(), e);
    ...
    public void SaveResponse(String path) {//Sauvegarde la réponse à l'emplacement path
        MoocPilotLogger.LOGGER.log(Level.INFO, "CsvTraitement.SaveResponse path=" + path + " & STATS");
...
//MoocPilotLogger.LOGGER.log(Level.INFO,"CsvTraitement.SetTabEleveIndex, i:"+i+", key:"+key);
                }
            }
        }
    ...
    private void stopWrongFiles() {
        MoocPilotLogger.LOGGER.log(Level.INFO, "CsvTraitement.stopWrongFiles, size=" + this.recordsList.size());
        CSVRecord lastColumn = this.recordsList.get(this.recordsList.size() - 1).get(0);
        MoocPilotLogger.LOGGER.log(Level.INFO, lastColumn.toString());
        for (int i = this.recordsList.size() - 2; i >= 0; i--) {
            if (this.recordsList.get(i).size() != 0) {
                ...
                MoocPilotLogger.LOGGER.log(Level.INFO, checkedColumn.toString());
...
*/
        ...
        MoocPilotLogger.LOGGER.log(Level.INFO, "CsvTraitement.SetListColumnName, columnUsername:" + this.columnUsername + ", columnCohorte:" + this.columnCohorte);
...
*/
                ...
                if (colName.equalsIgnoreCase("grade")) {
                    ...
                    listUtilsName.add(i);
                    columnGrade = i;
                    MoocPilotLogger.LOGGER.log(Level.INFO, "CsvTraitement.SetListColumnUtilsName,, add " + i + ": " + colName); // QCM, TP
                }
            }
        }
//~ MoocPilotLogger.LOGGER.log(Level.INFO,"CsvTraitement.SetListColumnUtilsName, columnWeek1 "+columnWeek1+", columnWeek2 "+columnWeek2+", listUtilsName "+listUtilsName);
            ...
            if (tabHashtable.get(i).containsKey(newId)) {
                ...
                for (int j = 0; j < this.listColumnUtilsName.size(); j++) {
                    ...
                    if (actualRow.get(this.listColumnUtilsName.get(j)).equals("Not Attempted") || actualRow.get(this.listColumnUtilsName.get(j)).equals("Not Available")) {
                        ...
                        //MoocPilotLogger.LOGGER.log(Level.INFO,actualRow.get(this.listColumnUtilsName.get(j)));
...
//MoocPilotLogger.LOGGER.log(Level.INFO,"CsvTraitement.SetEleve id="+id+", login="+login+", email="+email+", semaineInscription="+semaineInscription+", tabNotes=..."+", cohorte="+cohorte);
